...
context_plan_step (addmeth _ _) Action (active_agenda (Address::T)) (active_agenda (Address::T)) NewNodes Spypoint DepthLimit:-
	...
	retrieve_node Address K,
	(K = (and_node Goal Address MethodIn M Pre Tactic)),
	((verbose off, !); (verbose on,  maybe_pprint_goal Goal)),
	((verbose off, !); (verbose on, pprint_address Address Depth)),
...
pds_method_trans G (repeat_until C M) F 
                 (cond_meth C id_meth (then_meth L (repeat_until C M1))) :-
	keep_variables_free M M1,
	!, pds_method_trans G M F L.


%% Control Rule emulation.
pds_method_trans G (strat_meth CR MList) F Cont:-!,
	controll CR G MList NewMList, !,
	((verbose off, !); (verbose on,
	printstdout "Method List:",
	printstdout NewMList)),
	mapbuild orelse_meth NewMList OrElseExp, !,
	pds_method_trans G OrElseExp F Cont.

controll [] _ LM LM.
controll (H::T) G LM Out:-
        controll T G LM Out1,
        (control_rule_for _ H G Out1 Out, !; Out1 = Out).
...
reject ML1 ML2 NewML:-
	partition ML1 ML2 _ NewML.

prefer ML1 ML2 NewML:-
	partition ML1 ML2 ML3 ML4,
	append ML3 ML4 NewML.

replace M1 M2 [] [].
replace M1 M2 (H::T) (H::NewT):-
	not (M1 = H),
	replace M1 M2 T NewT.
replace M1 M2 (M1::T) (M2::NewT):-
	replace M1 M2 T NewT.

local partition (list A) -> (list A) -> (list A) -> (list A) -> o.
partition [] L [] L.
partition (H::T) List (H1::B) C:-
	keep_variables_free H H1,
	memb_and_rest H1 List Rest, !,
	partition T Rest B C.
partition (H::T) List B C:-
	keep_variables_free H H1,
	partition T List B C.
