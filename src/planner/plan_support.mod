...
apply_method_to_goal (critique Critic _) (critic_meth Critic) _ _ _.
...
evaluate_in_turn (H, T) ((success H)::Tout) Out:-
	(not (H = (_, _))),
	H,
	((verbose off, !); (verbose on, pprint_name H, pprint_string "success\n")),
	evaluate_in_turn T Tout Out.
evaluate_in_turn (H, T) ((failed H)::Tout) 0:-
	(not (H = (_, _))),
	(not H),
	((verbose off, !); (verbose on, pprint_name H, pprint_string "failed\n")),
	evaluate_in_turn T Tout _.
evaluate_in_turn Pre ((success Pre)::nil) 1:-
	(not (Pre = (_, _))),
	Pre, 
	((verbose off, !); (verbose on, pprint_name Pre, pprint_string "success\n")).
evaluate_in_turn Pre ((failed Pre)::nil) 0:-
	(not (Pre = (_, _))),
	((verbose off, !); (verbose on, pprint_name Pre, pprint_string "failed\n")),
	(not Pre).
