...
getrule R :- cfg Gs, ly_member R Gs.
...
% ly_append T NewHandles NewT,   % BFS
 ly_append NewHandles T NewT,   % DFS
...
rr_conflict (shand LB1 (rule (LHS ==> RHS) Act) LA1)
  ...
  ly_append RRH [B1|Alpha] VP1,        % first viable prefix
  ly_append RR2 [B2|Alpha2] VP1,       % second viable prefix
  ly_member B1 LB1, ly_member B2 LB2,     % pick two lookbacks
  ly_member A1 LA1, ly_member A2 LA2,     % even after lookahead consideration?    
...
sr_conflict LB1 Gamma Next 
  ...
  ly_append Gamma [Bf1|Alpha] VP1,      % (Gamma already reversed)
  ly_append RR2 [Bf2|Alpha2] VP1,       % second viable prefix      
  ly_member A2 LA2, 
...
%  RH2 = [Ea,OP1,Eb], 
  ly_append U [Ea,OP1,Eb] RH2,	  % operator may be at end of production 
...
gen_reduce OS (shand LB (rule (LHS ==> RHS) Act) LA) :-   % make rule clause
  ...
  output OS "\tly_member B (", printterm OS LB, 
  output OS "), ly_member Sym (", printterm OS LA, 
...
ly_append nil L L.
ly_append [H|T] L [H|Z] :- ly_append T L Z.
...
ly_nth 1 X [X|T].
ly_nth N X [H|T] :- not(X = H), N2 is (N - 1), ly_nth N2 X T.

ly_length [] 0.
ly_length [H|T] N :- ly_length T N2, N is (N2 + 1).

reverse nil nil.
reverse [H|T] M :- reverse T L, ly_append L [H] M.

ly_filter P nil nil.
ly_filter P [H|T] [H|R] :- (P H), !, ly_filter P T R.
ly_filter P [H|T] R :- ly_filter P T R.
...
% assumes iconst, sconst, and ly_id as grammar symbols:
freshcopy (iconst A) (iconst B) :- !.
freshcopy (sconst A) (sconst B) :- !.
freshcopy (ly_id A) (ly_id B) :- !.
...
%yyalphanum Str In Tok :- eof In, !, (printname XX Str Tok; Tok = (ly_id Str)).
yyalphanum Str In Tok :- lookahead In LA, eof_symbol LA, !,
   (printname XX Str Tok; Tok = (ly_id Str)).
...
print_tokens N [(ly_id A)|T] :- N > 0, !, print A,
