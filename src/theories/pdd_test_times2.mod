...
is_otype pdd_test_times2 nat [zero] [s].

has_otype pdd_test_times2 zero nat.
has_otype pdd_test_times2 s (arrow [nat] nat).
has_otype pdd_test_times2 plus (arrow [nat, nat] nat).
has_otype pdd_test_times2 minus (arrow [nat, nat] nat).
has_otype pdd_test_times2 otimes (arrow [nat, nat] nat).
has_otype pdd_test_times2 exp (arrow [nat, nat] nat).
has_otype pdd_test_times2 pdd_leq (arrow [nat, nat] bool).
has_otype pdd_test_times2 half (arrow [nat] nat).
has_otype pdd_test_times2 double (arrow [nat] nat).
has_otype pdd_test_times2 even (arrow [nat] bool).
...
has_otype pdd_test_times2 pred (arrow [nat] nat).
definition pdd_test_times2 pred1
	trueP
	(app pred [zero])
	zero.
definition pdd_test_times2 pred2
	trueP
	(app pred [(app s [X])])
	X.
...
lemma pdd_test_times2 neq_zero_s equiv trueP (app eq [zero, (app s _)]) 
                                        falseP.
lemma pdd_test_times2 neq_s_zero equiv trueP (app eq [(app s _), zero]) 
                                        falseP.
lemma pdd_test_times2 neq_eq equiv trueP (app neq [X, X]) falseP.


% plus
%
definition pdd_test_times2 plus1 trueP (app plus [zero, Y]) Y.
definition pdd_test_times2 plus2 trueP
     (app plus [(app s [Y]), X]) 
     (app s [(app plus [Y, X])]).

top_goal_c pdd_test_times2 simple [] (app eq [zero, (app plus [zero, (app plus [zero,  zero])])]) 
[(unsure plus [(rrstatus plus1 Tree1 [] Used1), 
               (rrstatus plus2 Tree2 [] Used2)])].

% simple pdd_test_times2 
%
top_goal_c pdd_test_times2 assp []
    (app forall [nat, 
...
% simple pdd_test_times2 
%
top_goal_c pdd_test_times2 comp []
     (app forall [nat,
...
top_goal_c pdd_test_times2 plus2right []
	(app forall [nat,
         (abs (y\ (app forall [nat,
	  (abs (x\ (app eq [
           (app plus [x, (app s [y])]),
...
top_goal_c pdd_test_times2 plus1lem []
...
top_goal_c pdd_test_times2 plusxx []
	(app forall [nat, (abs (x\
		(app eq [
			(app plus [(app s [x]), x]),
...
top_goal_c pdd_test_times2 zeroplus []
	(app forall [nat, (abs (x\
...
% times
%
definition pdd_test_times2 times1 trueP
   (app otimes [zero, X]) 
   zero.
definition pdd_test_times2 times2 trueP
   (app otimes [(app s [Y]), X]) 
   (app plus [(app otimes [Y, X]), Y]).

% simple pdd_test_times2 
%
top_goal_c pdd_test_times2 comm []
     (app forall [nat,
      (abs (x\ (app forall [nat, 
...
% simple pdd_test_times2 
%
top_goal_c pdd_test_times2 dist []
   (app forall [nat,
    (abs (x\ (app forall [nat,
     (abs (y\ (app forall [nat,
      (abs (z\ (app eq [
...
% simple pdd_test_times2, different argument order from dist
%
top_goal_c pdd_test_times2 distr []
   (app forall [nat,
    (abs (x\ (app forall [nat,
     (abs (z\ (app forall [nat,
...
% simple pdd_test_times2, different argument order from dist
%
top_goal_c pdd_test_times2 assm []
   (app forall [nat,
    (abs (z\ (app forall [nat,
     (abs (y\ (app forall [nat,
      (abs (x\ (app eq [
...
top_goal_c pdd_test_times2 zerotimes []
	(app forall [nat, (abs (x\
		(app forall [nat, (abs (y\
	(app imp [(app or [
				(app eq [x, zero]),
...
top_goal_c pdd_test_times2 times2right []
	(app forall [nat, (abs (x\
		(app forall [nat, (abs (y\
...
% exp
%
definition pdd_test_times2 exp1 trueP
   (app exp [_, zero]) 
   (app s [zero]).
definition pdd_test_times2 exp2 trueP
   (app exp [X, (app s [Y])]) 
   (app otimes [X, (app exp [X, Y])]).

% slightly more difficult pdd_test_times2
%
top_goal_c pdd_test_times2 expplus []
    (app forall [nat,
     (abs (z\ (app forall [nat,
      (abs (x\ (app forall [nat,
       (abs (y\ (app eq [
...
% slightly more difficult pdd_test_times2
%
top_goal_c pdd_test_times2 exptimes []
    (app forall [nat,
...
% s_functional
%
lemma pdd_test_times2 s_functional equiv trueP
...
axiom pdd_test_times2 pdd_leq1 equiv trueP
   (app pdd_leq [zero, Y])
   falseP.
axiom pdd_test_times2 pdd_leq_ss equiv trueP
   (app pdd_leq  [(app s [X]), (app s [Y])]) 
   (app pdd_leq [X, Y]).
lemma pdd_test_times2 pdd_leq_transitive equiv 
	trueP
...
%
top_goal_c pdd_test_times2 pdd_leqrefl []
   (app forall [nat,
...
% Arithmetic.
%
top_goal_c pdd_test_times2 pdd_leqsuc  []
    (app forall [nat,
...
top_goal_c pdd_test_times2 pdd_leqsum []
      (app forall [nat,
...
definition pdd_test_times2 double1 trueP
   (app double [zero])
   (app s [zero]).

definition pdd_test_times2 double2 trueP
(app double [(app s [X])])
(app s [(app double [X])]).

top_goal_c pdd_test_times2 doubleplus []
	(app forall [nat, (abs (x\
		(app eq [(app double [x]),
				(app plus [x, x])])) nat)])
...
top_goal_c pdd_test_times2 doubletimes []
	(app forall [nat, (abs (x\
		(app eq [(app double [x]),
...
top_goal_c pdd_test_times2 doubletimes2 []
	  (app forall [nat, (abs (x\
                (app eq [(app double [x]),
...
%
induction_scheme pdd_test_times2 nat_struct
   nat
...
induction_scheme pdd_test_times2 twostep
   nat
...
benchmark_plan pdd_test_times2 Meth Query :-
	       testloop (%interaction_mode command,
	       (set_theory_induction_scheme_list pdd_test_times2,
		...
		(add_theory_defs_to_sym_eval_list pdd_test_times2,
