...
tlbef Pos ((lbef Pos P1 P2) proves (Gamma --> Delta)) NewGoal :-
  NewGoal = (and_goal (P1 proves (((Tau eqdot Tau')::Gamma') --> Delta))
                      (P2 proves ((((succ_ Tau) bef Tau')::Gamma') --> Delta))),
  F = (Tau bef Tau'),
  memberN_ Pos F Gamma, deleteN_ Pos Gamma Gamma',
  print_ "**** Left-BEF (", write_ Pos, print_ ") gives: ",
  write_ NewGoal, print_ "\n".

trbef Pos ((rbef Pos P) proves (Gamma --> Delta)) NewGoal :-
  NewGoal = (P proves (Gamma --> (((succ_ Tau) bef Tau')::(Tau eqdot Tau')::Delta'))),
  F = (Tau bef Tau'),
  memberN_ Pos F Delta, deleteN_ Pos Delta Delta',
  print_ "**** Right-BEF (", write_ Pos, print_ ") gives: ",
  write_ NewGoal, print_ "\n".
...
twconn ((wconn P1 P2 P3 P4 P5) proves (Gamma --> Delta)) NewGoal :-
  NewGoal = (and_goal (P1 proves (Gamma --> ((T0 bef T1)::Delta) ))
            ...
            (and_goal (P3 proves ((((succ_ T1) bef T2)::Gamma) --> Delta ))
            (and_goal (P4 proves (((T1 eqdot T2)::Gamma) --> Delta ))
                      (P5 proves ((((succ_ T2) bef T1)::Gamma) --> Delta )) )))),
  print_ "**** WEAK CONNECTEDNESS gives: ", write_ NewGoal, print_ "\n".

tsconn T1 T2 ((sconn T1 T2 P1 P2 P3) proves (Gamma --> Delta)) NewGoal :-
  NewGoal = (and_goal (P1 proves ((((succ_ T1) bef T2)::Gamma) --> Delta ))
            (and_goal (P2 proves ((((succ_ T2) bef T1)::Gamma) --> Delta ))
...
%  print_ "Trying with ENTAILMENT on ", write_ (Gamma --> Delta), print_ "\n",
%  idfs_b 2 1 4  (tax _::trefl::nil)
