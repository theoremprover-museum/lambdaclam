...
check_ Proof Phi :-
  (repeat_tac (app_list!_tac (
    ...
    pre tluntil::pre truntil::pre tlboxt::pre trboxt::pre tlnext::pre trnext::
    trefleq::pre tsubeq::pre2 tsubeql::pre2 tsubeqr::pre2l tisol::close_tac::
    pre tlbef::pre trbef::
    tser::trefl::ttrans::twdir::tsdir::twconn::pret tsconn::tatom::
    pre2t trbox2::pre2t tldia2::pre2t tluntil2::pre2t trboxt2::nil)))
...
topS Gamma Delta :-
  tinter Tactic (Proof proves (Gamma --> Delta)) OutGoal, print_ "\n",
  (
    ( OutGoal = true_goal, trim Tactic Tactic',
      print_ "----> Proof found. Formula\n\t", write_ Phi, print_ "\n",
      print_ "----> is proved by tactic\n\t", write_ Tactic', print_ "\n",
...
tproc (then_tac (try_tac Tac) (pair_tac Tac1'
  ...
  not(Tac = then_tac _ _),
% print_ "three, Tac is ", write_ Tac,
...
% print_ "linear, Tac is ", write_ Tac,
